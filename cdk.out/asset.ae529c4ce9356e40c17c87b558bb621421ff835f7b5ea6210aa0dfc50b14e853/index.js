...
const removeProductFromTag = async (oldTags, oldTag, id) => {
  const index = oldTags.indexOf(oldTag);
  const params = {
    TableName: productTagsTable,
    Key: { TAG_NAME: },
    ExpressionAttributeValues: {},
    ExpressionAttributeNames: {},
    UpdateExpression: "REMOVE #c[:index]",
    ExpressionAttributeNames: {
      "#c": "products"
    },
    ExpressionAttributeValues: {
...
const main = async (event, context, callback) => {
  ...
  if (tags) {
    var response;
    try {
      response = getItemFromDynamoDB(id)
      console.log(`Successfuly retrieved item ${id} from dynamodb`);
    } catch (error) {

    }

    const oldTags = response.Item.PRODUCT_TAGS.L
    const oldTagsValues = oldTags.map((tag) => tag.S)

    for (const oldTag of oldTagsValues) {
      if (tags.includes(oldTag)) {
        continue;
      }

      try {
        await removeProductFromTag(oldTags, oldTag, id)
      } catch (error) {
