...
const main = async (event) => {
  ...
  if (productType === "DEAL") {
    ...
    if (dealPrice >= price) {
      return handleError({ message: dealPriceIsHigherThanPrice, statusoCode: 500 })
    }
  ...
  tags.forEach((tag) => {
    ...
    const promise = docClient.update(updateTagsParams);
    promises.push(promise)
  })

  try {
    await Promise.all(promises)
    console.log(`A tag dos produtos foram adicionadas na tabela de tags. ${data}`)
  } catch (error) {
    return handleError({ message: "Erro ao adicionar produto na tabela de tags", statusCode: error.statusCode })
  }
  ...
  try {
    ...
    return handleError({ message: "Erro ao adicionar imagem no s3 bucket", statusCode: error.statusCode })
  }
  ...
  try {
    ...
    return handleError({ message: "Erro ao adicionar imagem redimensionada no s3 bucket", statusCode: error.statusCode })
  }
