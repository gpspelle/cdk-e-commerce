...
const updateItemOnDynamoDB = async (item, idAttributeName) => {
  ...
  if (item.reorderImages) {
    ...
    productImagesResized = item.PRODUCT_IMAGES.map((image) => `https://${IMAGES_BUCKET}.s3.${REGION}.amazonaws.com/${item[idAttributeName]}/${encodeS3URI(image.name.replace(/ /g, ""))}-resized`);
  }
...
const main = async (event, context, callback) => {
  ...
  if (images) {
    ...
    // add resized images
    images.forEach((image) => {
      const S3EncodedKey = `${id}/${encodeS3URI(image.name.replace(/ /g, ""))}-resized`
      ...
      S3Client.upload(S3Params, function (error, data) {
        ...
        console.log(`Resized file uploaded to S3 bucket successfully. ${data.Location}`)
      })
    });
  }
