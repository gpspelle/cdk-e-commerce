...
const main = async (event) => {
  ...
  if (oldTags) {
    for (const oldTag of oldTags) {
      ...
      try {
        ...
        return handleError({ message: "Erro ao remover produto de uma tag", statusCode: error.statusCode })
      }
    }
  }
  ...
  if (images) {
    ...
    try {
      ...
      return handleError({ message: "Erro ao inserir imagem no s3 bucket", statusCode: error.statusCode });
    }
    ...
    try {
      ...
      return handleError({ message: "Erro ao inserir imagem redimensionada no s3 bucket", statusCode: error.statusCode });
    }
  }
