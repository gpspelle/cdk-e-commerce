...
const main = (event, context, callback) => {
  ...
  const productImages = images.forEach((image) => {
    const { key } = s3ParseUrl(image.name)
    const S3EncodedKey = encodeS3URI(key)
    return {
      S: `https://${bucketName}.s3.${REGION}.amazonaws.com/${S3EncodedKey}`,
    }
  })
